function bills() {
    ...
    this.billsBeforeParliament = function(callback) {
        var thisFunc = this;
        var db = this.mongoJs.connect(this.databaseUrl, this.collections);
        // For now only returning bills that have text for them
        db.bills.find({ hasHtml: true }, function(err, bills) {
        ...
        db.bills.find({ path: path }, function(err, bills) {
            if (typeof(callback) === "function") {
                if (err || !bills.length) {
                    ...
                    var bill = bills[0];
                    // Convert anchor links to links to elsewhere on this page.
